...
class AdapterPhoto(private val context: Context,
    ...
    override fun onBindViewHolder(holder: MediaViewHolder, position: Int) {
        ...
        launching(item.media1?.uri, item.media1?.type, holder.image1)
        launching(item.media2?.uri, item.media2?.type, holder.image2)
        launching(item.media3?.uri, item.media3?.type, holder.image3)
    }

    private fun launching(itemUri: String?, itemType: MediaType?, holderImage: ImageView){
